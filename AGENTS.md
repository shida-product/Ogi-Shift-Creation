...
4. 明示されたGoal / DoDを起点にする。汎用的な「再開」は最近のGit履歴・差分・実装／研究成果・採択済み決定から復元し、履歴だけを着手許可としない。復元できない意図・人間待ち・外部状態だけ[.agent/handover.md](.agent/handover.md)の該当箇所で補う。不明な次単位は依存編集前に確認する。
5. 作る前に`rg`等で既存の責務・呼出元・検査を探す。参照入口は[README.md](README.md)、[.agent/lessons.md](.agent/lessons.md)。仕様・教訓・workflowは作業に必要な箇所だけ取得し、下位ディレクトリのAGENTSと必須gateを確認する。安全条件を読取削減のために省かない。
...
実スタッフの希望休・シフトを扱う。既存の勤務条件を推測で変更せず、必要な仕様と生成ロジックを照合する。実スタッフデータ・本番Supabase・定期実行設定を起動確認だけを目的に操作しない。
